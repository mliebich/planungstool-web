			...
			<main className="max-w-7xl mx-auto px-4 py-6">
				<div className="bg-white rounded-xl shadow-sm overflow-x-auto">
					<table className="w-full border-collapse">
						<thead>
							<tr>
								<th className="p-3 border-b text-left" style={{ backgroundColor: 'var(--gray-50)', width: '100px' }}>
									Zeit
								</th>
								{DAYS_OF_WEEK.map(day => (
									<th key={day} className="p-3 border-b text-center font-semibold" style={{ backgroundColor: 'var(--gray-50)' }}>
										{day}
									</th>
								))}
							</tr>
						...
						<tbody>
							{TIME_SLOTS.map(timeSlot => (
								<tr key={timeSlot}>
									<td className="p-2 border-b text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>
										{timeSlot}
									</td>
									{DAYS_OF_WEEK.map((_, dayIndex) => {
										const lesson = getLessonForSlot(dayIndex, timeSlot);
										const bgColor = lesson?.class ? getClassColor(lesson.class, customColors) : 'var(--gray-100)';

										return (
											<td
												key={dayIndex}
												className="p-1 border-b border-r cursor-pointer hover:opacity-80 transition-opacity"
												onClick={() => lesson ? openEditModal(lesson) : handleTimeSlotClick(dayIndex, timeSlot)}
											>
												{lesson ? (
													<div
													...
													>
														...
														{lesson.class && <div className="text-xs opacity-90">{lesson.class}</div>}
														{lesson.room && <div className="text-xs opacity-75">{lesson.room}</div>}
													</div>
												) : (
													<div className="min-h-[60px] rounded-lg" style={{ backgroundColor: 'var(--gray-50)' }} />
												)}
											</td>
										);
									})}
				...
				<div className="mt-6 p-4 bg-white rounded-xl shadow-sm">
					<h3 className="font-semibold mb-3" style={{ color: 'var(--text-primary)' }}>Legende</h3>
